    ...
    def decode_batch_predictions(
        ...
        for j, (img_predictions, orig_size) in enumerate(
            ...
            for z, row_prediction in enumerate(row_predictions):
                ...
                spanning_cells = []
                for span_cell_idx, spanning_cell in enumerate(row_cell_predictions[z]):
                    ...
                    for zz, spanning_cell in enumerate(spanning_cells):
                        ...
                        correct_col_width = sum(
                            [
                                col.width
                                for col in columns[
                                    col_idx : col_idx + spanning_cell.colspan
                                ]
                            ]
                        )
            ...
            for z, grid_row in enumerate(grid_cells[1:]):
                ...
                for cell_idx, cell in enumerate(grid_row):
                    if cell_idx >= len(prev_row):
                        continue

                    above_cell = prev_row[cell_idx]
                    if all(
                        [
